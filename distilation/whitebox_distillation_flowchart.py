...
# Создаем фигуру для блок-схемы white-box дистилляции
...
# Создаем блоки для White-Box дистилляции
# Начало
create_block(5, 10, 3, 0.8, colors['start_end'], 'Начало White-Box Дистилляции', fontsize=12)
...
create_block(2, 4, 3, 0.8, colors['teacher'], 'Получение логитов\nот модели учителя', fontsize=11)
...
create_decision(5, -2, 3, 0.8, 'Все эпохи\nзавершены?', fontsize=11)

# Сохранение модели
create_block(5, -3, 3, 0.8, colors['student'], 'Сохранение\nмодели ученика', fontsize=11)

# Конец
create_block(5, -4, 3, 0.8, colors['start_end'], 'Конец', fontsize=12)
...
create_arrow((9.5, -1.6), (9.5, 6.4))
create_arrow((9.5, 6.4), (6.5, 6.4))
create_arrow((6.5, -3), (6.5, -3.2))
create_arrow((6.5, -4), (6.5, -4.2))

# Добавляем пояснения
ax.text(2, 2.4, 'KL дивергенция между\nлогитами учителя и ученика', ha='center', va='center', 
...
# Заголовок
ax.set_title('Блок-схема White-Box дистилляции', fontsize=16, fontweight='bold', pad=20)

# Сохраняем блок-схему
plt.tight_layout()
plt.savefig('whitebox_distillation_flowchart.png', dpi=300, bbox_inches='tight')
plt.close()

print("Блок-схема White-Box дистилляции создана и сохранена как 'whitebox_distillation_flowchart.png'")